...
import { MinioPath, MinioObject } from '@/types/minio';
import { listBuckets, listObjects, uploadFile, downloadFile, deleteFile } from '@/lib/minio-client';
...
export default function MinioExplorer() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        await uploadFile(currentBucket, objectName, file, (progress) => {
  ...
  return (
    <div className="h-full w-full flex flex-col">
      ...
      />
      {/* 主标题栏 */}
      <div className="w-full h-[60px] border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 fixed top-0 left-0 right-0 z-50">
          ...
          <div className="flex items-center justify-between w-full h-full">
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon">
                    <LucideReact.Sun className="h-5 w-5 rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-0" />
                    <LucideReact.Moon className="absolute h-5 w-5 rotate-90 scale-0 transition-all dark:rotate-0 dark:scale-100" />
      ...
      <div className="flex-1 flex min-h-0 w-full mt-[60px]">
        {/* 可折叠侧边栏 */}
        <div className={`${isSidebarCollapsed ? 'w-0 opacity-0' : 'w-[250px] opacity-100'} flex-none border-r bg-muted/10 transition-all duration-300 overflow-hidden`}>
